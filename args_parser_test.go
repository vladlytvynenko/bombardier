...
func TestParsePrintSpec(t *testing.T) {
	exps := []struct {
		...
		},
		{
			...
			[4]bool{},
			fmt.Errorf(
				"Spec %q has too many parts, at most 4 are allowed", "i,p,r,s,i",
			),
		},
		{
			"i",
	...
	for _, e := range exps {
		...
		act[0], act[1], act[2], act[3], err = parsePrintSpec(e.spec)
